...
const createGrantRoutes = (deps) => {
    ...
    return {
        request: async ({ url }, args, clientOverride) => {
            if (!args.access_token && !args.subject) {
                throw new _1.OpenPaymentsClientError('Invalid Grant Request', {
                    description: 'Grant request must include at least one of "access_token" or "subject".'
                });
            }
            if (args.access_token) {
                const outgoingPaymentAccess = args.access_token.access.find((el) => el.type === 'outgoing-payment');
                if (outgoingPaymentAccess?.limits
                    ?.debitAmount &&
                    outgoingPaymentAccess?.limits
                        ?.receiveAmount) {
                    throw new _1.OpenPaymentsClientError('Invalid Grant Request', {
                        description: 'Only one of "debitAmount" or "receiveAmount" may be specified.'
                    });
                }
            }
            return (0, requests_1.post)(baseDeps, {
                url,
                body: {
                    ...args,
